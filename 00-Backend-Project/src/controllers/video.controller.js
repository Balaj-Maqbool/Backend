...
const uploadAVideo = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(200, createdVideo, "Video File Uploaded Successfully!!"))
...
const deleteAVideo = asyncHandler(async (req, res) => {
    ...
    const deletedVideo = await Video.findByIdAndDelete(video._id).select(" title description duration -_id ")
    if (!deletedVideo) {
        throw new ApiError(404, "No video was found to delete")
    }

    return res.status(200).json(new ApiResponse(200, deletedVideo, "Video deleted Successfully from the DB"))
})
