...
const createPlaylist = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(200, newPlayList, "A playlist created Successfully"))
...
const getUserPlaylists = asyncHandler(async (req, res) => {
    ...
    const allPlayLists = await PlayList.aggregatePaginate(
        PlayList.aggregate(pipeline), options
    )
...
const getPlaylistById = asyncHandler(async (req, res) => {
        ...
        [
            {
                $match: {
                    $and: [
                        { _id: new Types.ObjectId(playlistId?.trim()) },
                        { playListOwner: userId }
                    ]
                }
            },
    ...
    return res.status(200).json(new ApiResponse(200, playlist[0], "A Playlist fetched Successfully!!"))
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, videoAddedPlaylist, "Video Added  to the Playlist"))
})
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    ).populate("playListOwner", "_id username").select("_id name videos")

    if (!videoRemovedPlaylist) {
        throw new ApiError(403, "Playlist Does not exist in the DB")
    }

    return res.status(200).json(new ApiResponse(200, videoRemovedPlaylist, "Video from the Playlist is deleted Successfully"))
})
...
const deletePlaylist = asyncHandler(async (req, res) => {
    ...
    ).populate("playListOwner", "username email _id ").select("name description playListOwner")
    if (!playlist) {
        throw new ApiError(403, " Unauthorized Access, Wrong Id or Wrong Owner")
    }

    return res.status(204).json(new ApiResponse(200, playlist, "Playlist Deleted successfully !!!"))
